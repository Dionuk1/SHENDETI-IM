(function initializeShendetiTheme(window, document) {
    ...
    function toggleTheme() {
        return selectTheme(document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark');
    }

    applyTheme(storedTheme());

    window.ShendetiTheme = Object.freeze({
    ...
    window.addEventListener('storage', (event) => {
        if (event.key === STORAGE_KEY) selectTheme(event.newValue);
    });
})(window, document);
